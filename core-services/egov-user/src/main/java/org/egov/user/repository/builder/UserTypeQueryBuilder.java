...
public class UserTypeQueryBuilder {
	...
	private static final String SELECT_USER_QUERY = "SELECT userdata.title, userdata.salutation, userdata.dob, userdata.locale, userdata.username, userdata"
			...
			+ ".bloodgroup, userdata.photo, userdata.identificationmark,  userdata.tenantid, userdata.id, userdata.uuid, userdata.alternatemobilenumber, userdata.validitydate, userdata.usertenantid, addr.id as addr_id, addr.type as "
			+ "addr_type, addr.address as addr_address,  addr.city as addr_city, addr.pincode as addr_pincode, addr"
			+ ".tenantid as addr_tenantid, addr.userid as addr_userid, ur.role_code as role_code, ur.role_tenantid as role_tenantid \n"
	...
	private static final String PAGINATION_WRAPPER = "SELECT * FROM "
			+ "(SELECT *, DENSE_RANK() OVER (ORDER BY id) offset_ FROM " + "({baseQuery})" + " result) result_offset "
	...
	private void addWhereClause(final StringBuilder selectQuery, final List preparedStatementValues,
		...
		if (userSearchCriteria.getId() != null && !userSearchCriteria.getId().isEmpty()) {
			...
			selectQuery.append(" userdata.id IN ( ")
					.append(getQueryForCollection(userSearchCriteria.getId(), preparedStatementValues)).append(" )");
		}
...
//        }
		...
		if (!isEmpty(userSearchCriteria.getUuid())) {
			isAppendAndClause = addAndClauseIfRequired(isAppendAndClause, selectQuery);
			selectQuery.append(" userdata.uuid IN (")
					.append(getQueryForCollection(userSearchCriteria.getUuid(), preparedStatementValues)).append(" )");
		}
...
//        }
	...
	private void addOrderByClause(final StringBuilder selectQuery, final UserSearchCriteria userSearchCriteria) {
		final String sortBy = userSearchCriteria.getSort() != null && !userSearchCriteria.getSort().isEmpty()
				? " userdata." + userSearchCriteria.getSort().get(0)
				: "userdata.name";
	...
	private void addWhereClauseUserRoles(final StringBuilder selectQuery, final List preparedStatementValues,
		...
		if (!isEmpty(userSearchCriteria.getRoleCodes())) {
			isAppendAndClause = addAndClauseIfRequired(isAppendAndClause, selectQuery);
			selectQuery.append(" ur.role_code IN (")
					.append(getQueryForCollection(userSearchCriteria.getRoleCodes(), preparedStatementValues))
					.append(" )");
		}
	...
	/**
	 * This method is always called at the beginning of the method so that and is
	 * prepended before the field's predicate is handled.
	 *
	...
	public String getInsertUserQuery() {
		return "insert into eg_user (id,uuid,tenantid,salutation,dob,locale,username,password,pwdexpirydate,mobilenumber,altcontactnumber,emailid,active,name,gender,pan,aadhaarnumber,"
				+ "type,guardian,guardianrelation,signature,accountlocked,bloodgroup,photo,identificationmark,createddate,lastmodifieddate,createdby,lastmodifiedby,alternatemobilenumber,validitydate,usertenantid) values (:id,:uuid,:tenantid,:salutation,"
				...
				+ ":accountlocked,:bloodgroup,:photo,:identificationmark,:createddate,:lastmodifieddate,:createdby,:lastmodifiedby,:alternatemobilenumber,:validitydate,:usertenantid) ";
	}
