...
@Service
public class ArchitectDetailsResultSetExtractor implements ResultSetExtractor<List<User>> {

    private ObjectMapper objectMapper;

    @Autowired
    ArchitectDetailsResultSetExtractor(ObjectMapper objectMapper) {
        ...
        while (rs.next()) {
            ...
            if (!usersMap.containsKey(userId)) {
                ...
                user = User.builder().id(rs.getLong("id")).tenantId(rs.getString("tenantid")).title(rs.getString("title"))
                        ...
                        .accountLockedDate(rs.getLong("accountlockeddate")).alternateMobileNumber(rs.getString("alternatemobilenumber")).validityDate(rs.getTimestamp("validitydate")).userTenantid(rs.getString("user_tenantid"))
                        .build();
