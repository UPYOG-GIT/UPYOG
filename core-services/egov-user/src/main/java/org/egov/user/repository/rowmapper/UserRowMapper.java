...
public class UserRowMapper implements RowMapper<User> {
    ...
    public User mapRow(final ResultSet rs, final int rowNum) throws SQLException {
        final User user = User.builder().id(rs.getLong("id")).tenantId(rs.getString("tenantid")).title(rs.getString("title")).salutation(rs.getString("salutation"))
                ...
                .accountLocked(rs.getBoolean("accountlocked")).photo(rs.getString("photo")).identificationMark(rs.getString("identificationmark")).uuid(rs.getString("uuid")).alternateMobileNumber(rs.getString("alternatemobilenumber")).validityDate(rs.getTimestamp("validitydate")).userTenantid(rs.getString("usertenantid")).build();
