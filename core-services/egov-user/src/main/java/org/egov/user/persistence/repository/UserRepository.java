...
//		};
	...
	private void saveUserRoles(User entityUser) {
		...
		for (Role role : entityUser.getRoles()) {
			...
			log.info("processedRoleTenantId: " + processedRoleTenantId);
			log.info("role.getCode(): " + role.getCode());
			log.info("role.getTenantId(): " + role.getTenantId());
			batchValues.add(new MapSqlParameterSource("role_code", role.getCode())
					.addValue("role_tenantid", processedRoleTenantId).addValue("user_id", entityUser.getId())
					.addValue("user_tenantid", entityUser.getTenantId()).addValue("lastmodifieddate", new Date())
					.getValues());
//            batchValues.add(
