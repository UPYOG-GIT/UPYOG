...
public class TransactionRowMapper implements RowMapper<Transaction> {
	...
	public Transaction mapRow(ResultSet resultSet, int i) throws SQLException {
		...
		User user = User.builder().uuid(resultSet.getString("user_uuid")).userName(resultSet.getString("user_name"))
				.mobileNumber(resultSet.getString("mobile_number")).emailId(resultSet.getString("email_id"))
				.name(resultSet.getString("name")).tenantId(resultSet.getString("user_tenant_id")).build();
		...
		return Transaction.builder().txnId(resultSet.getString("txn_id")).txnAmount(resultSet.getString("txn_amount"))
				...
				.billId(resultSet.getString("bill_id")).productInfo(resultSet.getString("product_info")).user(user)
				...
				.gatewayStatusMsg(resultSet.getString("gateway_status_msg")).receipt(resultSet.getString("receipt"))
				.consumerCode(resultSet.getString("consumer_code")).additionalDetails(additionalDetails)
				.taxAndPayments(taxAndPayments).auditDetails(auditDetails).build();
	}
}
