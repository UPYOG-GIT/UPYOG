...
public class RazorPayGateway implements Gateway {
    ...
    private String CHECKOUT_URL; // Changed to non-final since assigned in constructor
    ...
    public RazorPayGateway(RestTemplate restTemplate, Environment environment, ObjectMapper objectMapper,
        ...
        this.CHECKOUT_URL = environment.getRequiredProperty("razorpay.url.checkout"); // Assigned here
    ...
    public String generateRedirectFormData(Transaction transaction) {
        try {
            ...
            // Fixed: Amount should be in paise as Long, not String in rupees
            String amtAsPaise = Utils.formatAmtAsPaise(transaction.getTxnAmount());
            options.put(TRANSACTION_AMOUNT_KEY, Long.valueOf(amtAsPaise)); // Changed to Long
            ...
            options.put("callbackUrl", callbackUrl); // Fixed: was "callbackUrl" but should be "callback_url" for Razorpay
    ...
    public String createRazorpayOrder(Transaction transaction) {
        try {
            // Set gateway details first
            setGatewayDetails(transaction.getTenantId());
            ...
            String amtAsPaise = Utils.formatAmtAsPaise(transaction.getTxnAmount());
            orderRequest.put("amount", Long.valueOf(amtAsPaise)); // Fixed: Amount must be Long
            orderRequest.put("currency", "INR");
            orderRequest.put("receipt", transaction.getTxnId());
    ...
    private boolean verifySignature(String orderId, String paymentId, String signature) {
        try {
            ...
            SecretKeySpec secretKeySpec = new SecretKeySpec(WORKING_KEY.getBytes(StandardCharsets.UTF_8), "HmacSHA256"); // Fixed: Use WORKING_KEY
