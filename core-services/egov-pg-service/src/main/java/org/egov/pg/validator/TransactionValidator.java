...
//		log.info("Optional :" + optional.get());
		...
		log.info("criteria.getTxnId(): " + criteria.getTxnId());
	...
	private void validateIfTxnExistsForBill(TransactionRequest transactionRequest, Map<String, String> errorMap) {
		...
		long criteriaCreatedTime = criteria.getCreatedTime();
		long thirtyMinutesInMillis = 30 * 60 * 1000;

		log.info("createdTime: " + criteriaCreatedTime);
		log.info("currentTimeMillis: " + currentTime);
		
		for (Transaction curr : existingTxnsForBill) {
			if (curr.getTxnStatus().equals(Transaction.TxnStatusEnum.PENDING)
					&& !(currentTime - criteriaCreatedTime > thirtyMinutesInMillis)) {
	...
	private void isGatewayActive(Transaction transaction, Map<String, String> errorMap) {
		log.info("transaction.getGateway(): " + transaction.getGateway());
