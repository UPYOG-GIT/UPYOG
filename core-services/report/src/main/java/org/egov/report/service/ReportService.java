...
public class ReportService {
		...
		try {
			...
			for (SourceColumn cd : reportDefinition.getSourceColumns()) {
				...
				if (cd.getType() != null) {
					...
					throw new CustomException("INVALID_TYPE_OF_SOURCE_COLUMN",
							"Type parameter in report definition is invalid for source column");
				}
			...
			for (SearchColumn cd : reportDefinition.getSearchParams()) {
				...
				if (cd.getType() != null) {
					...
					throw new CustomException("INVALID_TYPE_OF_SEARCH_PARAM",
							"Type parameter in report definition is invalid for search param");
				}
			}
	...
	public ResponseEntity<?> getReportDataSuccessResponse(final List<ReportResponse> reportResponse,
			final RequestInfo requestInfo, String tenantId) {
	...
	public ResponseEntity<?> getFailureResponse(final RequestInfo requestInfo, String tenantID) {
		final MetadataResponse metadataResponses = new MetadataResponse();
	...
	public ResponseEntity<?> getFailureResponse(final RequestInfo requestInfo, String tenantID, Exception e) {
		final MetadataResponse metadataResponses = new MetadataResponse();
		final ResponseInfo responseInfo = responseInfoFactory.createResponseInfoFromRequestInfo(requestInfo, false);
	...
	public List<ReportResponse> getAllReportData(ReportRequest reportRequest, String moduleName, String authToken) {
		...
		log.info("reportResponse.size(): " + reportResponse.size());
		log.info("reportResponse: " + reportResponse.toString());
	...
	public ReportResponse getReportData(ReportRequest reportRequest, String moduleName, String reportName,
		...
		log.info("maps.get(0).toString() -------------" + maps.get(0).toString());
	...
	private void populateData(List<SourceColumn> columns, List<Map<String, Object>> maps,
		...
		for (int i = 0; i < maps.size(); i++) {
			...
			newMap.putAll(map);
			// restore ['abc','xyz'] -> 'abc, xyz' -- earlier the string had to be
			// transformed to array to allow decryption incase of encrypted columns
			for (SourceColumn sourceColm : columns) {
				if (sourceColm.getType().toString().equals("stringarray")
						&& (newMap.get(sourceColm.getName()) != null)) {
	...
	private void populateReportHeader(ReportDefinition reportDefinition, ReportResponse reportResponse) {
		...
		// Let's check whether there's a linked report, we will set the default value in
		// header columns according to that
		...
		List<ColumnDetail> columnDetails = columns.stream()
				...
				.map(p -> new ColumnDetail(p.getShowColumn(), p.getLabel(), p.getType(), p.getDefaultValue(),
						p.getTotal(), p.getName(), p.getLocalisationRequired(), p.getLocalisationPrefix(),
						p.getIsMandatory(), p.getRowTotal(), p.getColumnTotal(), p.getInitialValue(), p.getMinValue(),
						p.getMaxValue()))
		...
		if (userInfo.getRoles() != null) {
			for (org.egov.common.contract.request.Role role : userInfo.getRoles()) {
				org.egov.common.contract.request.Role newRole = org.egov.common.contract.request.Role.builder()
						.code(role.getCode()).name(role.getName()).id(role.getId()).build();
				newRoleList.add(newRole);
			}
		}

		if (newRoleList.stream().filter(role -> (role.getCode() != null) && (userInfo.getType() != null)
				&& role.getCode().equalsIgnoreCase(userInfo.getType())).count() == 0) {
			org.egov.common.contract.request.Role roleFromtype = Role.builder().code(userInfo.getType())
					.name(userInfo.getType()).build();
