  ...
  const goNext = () => {
    ...
    if (ismultiple == true && fields.length == 1) {
      ...
      if (!formData?.id) {
        ...
        payload.swsUnitRegistrationNo = formData?.data?.edcrDetails?.planDetail?.planInformation?.swsUdyamAkankshaNumber;
        payload.swsServiceId = formData?.data?.edcrDetails?.planDetail?.planInformation?.swsServiceId;;
