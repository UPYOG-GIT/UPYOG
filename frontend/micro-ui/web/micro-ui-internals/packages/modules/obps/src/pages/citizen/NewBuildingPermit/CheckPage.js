...
//  console.log("paymentDetails" +  JSON.stringify(paymentDetails))
  ...
  <StatusTable  style={{border:"none"}}>
    ...
    <StatusTable>
    <Row className="border-none" label={t("BPA_TOTAL_BUILT_UP_AREA_HEADER")} text={`${datafromAPI?.planDetail?.virtualBuilding?.totalBuitUpArea} ${t("BPA_SQ_MTRS_LABEL")}`}></Row>
    ...
    <StatusTable>
        ...
        <Row className="border-none" label={t(`BPA_PROPOSED_SITE_ADDRESS`)} text={address?.address || t("CS_NA")} />
        {/* <Row className="border-none" label={t(`BPA_GIS_PLACE_NAME`)} text={address?.address?.geoLocation?.gisPlaceName || t("CS_NA")} /> */}
    </StatusTable>
    </Card>
    <Card style={{paddingRight:"16px"}}>
