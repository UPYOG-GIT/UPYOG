...
const CheckPage = ({ onSubmit, value }) => {
  ...
  if (value.businessService === "BPA_LOW") BusinessService = "BPA.NC_APP_FEE";
  ...
  return (
    <React.Fragment>
      ...
      <Card style={{paddingRight:"16px"}}>
        ...
        <StatusTable>
          <Row className="border-none" label={t("BPA_TOTAL_BUILT_UP_AREA_HEADER")} text={datafromAPI?.planDetail?.blocks?.[0]?.building?.totalBuitUpArea ? `${datafromAPI?.planDetail?.virtualBuilding?.totalBuitUpArea} ${t("BPA_SQ_MTRS_LABEL")}` : t("NA")}></Row>
