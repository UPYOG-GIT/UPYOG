...
public class PaymentRepository {
	...
	public PaymentRepository(NamedParameterJdbcTemplate namedParameterJdbcTemplate,
			PaymentQueryBuilder paymentQueryBuilder, PaymentRowMapper paymentRowMapper, BillRowMapper billRowMapper) {
	...
	public void savePayment(Payment payment) {
		try {
			...
			namedParameterJdbcTemplate.batchUpdate(INSERT_PAYMENTDETAIL_SQL,
					paymentDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(INSERT_BILL_SQL, billSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(INSERT_BILLDETAIL_SQL,
					billDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(INSERT_BILLACCOUNTDETAIL_SQL,
					billAccountDetailSource.toArray(new MapSqlParameterSource[0]));
	...
	public List<Map<String, Object>> getFeeDetail(String billId) {
		String query = paymentQueryBuilder.getFeeDetailSearchQuery(billId);
		log.info("getFeeDetail query: " + query);
...
//		return null;
	...
	public List<Payment> fetchPaymentsForPlainSearch(PaymentSearchCriteria paymentSearchCriteria) {
		Map<String, Object> preparedStatementValues = new HashMap<>();
		String query = paymentQueryBuilder.getPaymentSearchQueryForPlainSearch(paymentSearchCriteria,
	...
	public void updateStatus(List<Payment> payments) {
		...
		try {
			...
			namedParameterJdbcTemplate.batchUpdate(COPY_PAYMENT_SQL,
					paymentSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(COPY_PAYMENTDETAIL_SQL,
					paymentDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(COPY_BILL_SQL, billSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(STATUS_UPDATE_PAYMENT_SQL,
					paymentSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(STATUS_UPDATE_PAYMENTDETAIL_SQL,
					paymentDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(STATUS_UPDATE_BILL_SQL,
					billSource.toArray(new MapSqlParameterSource[0]));
		} catch (Exception e) {
	...
	public void updatePayment(List<Payment> payments) {
		...
		try {
			...
			namedParameterJdbcTemplate.batchUpdate(UPDATE_PAYMENT_SQL,
					paymentSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(UPDATE_PAYMENTDETAIL_SQL,
					paymentDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(UPDATE_BILL_SQL, billSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(UPDATE_BILLDETAIL_SQL,
					billDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(COPY_PAYMENT_SQL,
					paymentSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(COPY_PAYMENTDETAIL_SQL,
					paymentDetailSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(COPY_BILL_SQL, billSource.toArray(new MapSqlParameterSource[0]));
			namedParameterJdbcTemplate.batchUpdate(COPY_BILLDETAIL_SQL,
					billDetailSource.toArray(new MapSqlParameterSource[0]));
		} catch (Exception e) {
	...
	public void updateFileStoreId(List<Map<String, String>> idToFileStoreIdMaps) {
		...
		namedParameterJdbcTemplate.batchUpdate(FILESTOREID_UPDATE_PAYMENT_SQL,
				fileStoreIdSource.toArray(new MapSqlParameterSource[0]));
	...
	public List<String> fetchPaymentIds(PaymentSearchCriteria paymentSearchCriteria) {
		...
		return namedParameterJdbcTemplate.query(
				"SELECT id from egcl_payment ORDER BY createdtime offset " + ":offset " + "limit :limit",
				preparedStatementValues, new SingleColumnRowMapper<>(String.class));
	...
	public List<String> fetchPaymentIdsByCriteria(PaymentSearchCriteria paymentSearchCriteria) {
		...
		log.info(" fetchPaymentIdsByCriteria query: " + query);
	...
	public List<String> fetchIfsccode() {
		...
		return namedParameterJdbcTemplate.query(
				"SELECT distinct ifsccode from egcl_payment where ifsccode is not null ",
				new SingleColumnRowMapper<>(String.class));
	...
	public void updatePaymentBankDetail(JsonNode additionaldetails, String ifsccode) {
		...
		/**
		 * UPDATE_PAYMENT_BANKDETAIL_SQL query adds the bankdetails data to existing
		 * object type additionaldetails ex: object type additionaldetails data
		 * {"isWhatsapp": false }
		 */
		...
		/**
		 * UPDATE_PAYMENT_BANKDETAIL_EMPTYADDTL_SQL query update the bankdetails to
		 * empty/null additionaldetails. ex: empty or 'null' additionaldetails data.
		 */
		...
		/**
		 * UPDATE_PAYMENT_BANKDETAIL_ARRAYADDTL_SQL query adds bankdetails data to
		 * existing array type additionaldetails. ex: array additional data
