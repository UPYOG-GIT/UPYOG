...
//	private PaymentRepository paymentRepository;
	...
	private void addChildrenToPayment(ResultSet rs, Payment payment) throws SQLException {
		...
		if (paymentDetail == null) {
			...
			/*
			 * Long billDate = rs.getLong("billdate"); if(rs.wasNull()){billDate = null;}
			 * 
			 * AuditDetails billAuditDetails =
			 * AuditDetails.builder().createdBy(rs.getString("bill_createdby"))
			 * .createdTime(rs.getLong("bill_createdTime"))
			 * .lastModifiedBy("bill_lastmodifiedby")
			 * .lastModifiedTime(rs.getLong("bill_lastModifiedTime")) .build();
			 */

			/*
			 * List<String> collectionModesAllowedList = new LinkedList<>(); if(null !=
			 * rs.getString("collectionmodesnotallowed")) { String[] collectionModesAllowed
			 * = rs.getString("collectionmodesnotallowed").split(",");
			 * if(collectionModesAllowed.length!=0) collectionModesAllowedList =
			 * Arrays.asList(collectionModesAllowed); }
			 */
...
//			List<Map<String, Object>> feeDetailList = paymentRepository.getFeeDetail(billId);
		...
		/*
		 * PGobject billAccountDetailAdditionalObj = (PGobject)
		 * rs.getObject("bacdt_additionalDetails");
		 * 
		 * Integer order = rs.getInt("order"); if(rs.wasNull()){order = null;}
		 * 
		 * BillAccountDetail billAccountDetail = BillAccountDetail.builder()
		 * .id(rs.getString("bacdt_id")) .tenantId(rs.getString("bacdt_tenantid"))
		 * .billDetailId(rs.getString("billdetailid"))
		 * .demandDetailId(rs.getString("demanddetailid")) .order(order)
		 ...
		 * .additionalDetails(getJsonValue(billAccountDetailAdditionalObj)) .build();
		 * 
		 * 
		 * PGobject billDetailAdditionalObj = (PGobject)
		 * rs.getObject("bd_additionalDetails");
		 * 
		 * BillDetail billDetail = BillDetail.builder() .id(rs.getString("bd_id"))
		 * .tenantId(rs.getString("bd_tenantid")) .demandId(rs.getString("demandid"))
		 * .billId(rs.getString("billid")) .amount(rs.getBigDecimal("amount"))
		 ...
		 * .cancellationRemarks(rs.getString("cancellationremarks")) .build();
		 * 
		 * // Adding to Bill billDetail.addBillAccountDetail(billAccountDetail);
