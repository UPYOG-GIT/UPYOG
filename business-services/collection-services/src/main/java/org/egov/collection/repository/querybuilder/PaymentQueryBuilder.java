...
public class PaymentQueryBuilder {
	 ...
	 * "SELECT DISTINCT py.id as id,py.transactiondate as date " +
	 * " FROM egcl_payment py  " +
	...
	private static final String PAGINATION_WRAPPER = "SELECT * FROM "
			+ "(SELECT *, DENSE_RANK() OVER (ORDER BY py_id) offset_ FROM " + "({baseQuery})"
	...
	public static final String INSERT_BILLDETAIL_SQL = "INSERT INTO egcl_billdetial("
			...
			+ "            toperiod, additionaldetails," + "            channel, voucherheader, boundary,"
			+ "            collectiontype,"
	...
	public static MapSqlParameterSource getParametersForPaymentDetailCreate(String paymentId,
	...
	public static MapSqlParameterSource getParamtersForBillCreate(Bill bill) {
		...
		sqlParameterSource.addValue("collectionmodesnotallowed",
	...
	public String getPaymentCountQuery(String tenantId, String businessService,
	...
	private static void addWhereClause(StringBuilder selectQuery, Map<String, Object> preparedStatementValues,
		 ...
		 * !searchCriteria.getReceiptNumbers().isEmpty()) {
		 * addClauseIfRequired(preparedStatementValues, selectQuery);
		 * selectQuery.append(" pyd.receiptNumber IN (:receiptnumber)  ");
		 * preparedStatementValues.put("receiptnumber",
		 * searchCriteria.getReceiptNumbers()); }
		 * 
		 * if (!CollectionUtils.isEmpty(searchCriteria.getBusinessServices())) {
		 * addClauseIfRequired(preparedStatementValues, selectQuery);
		 * selectQuery.append(" pyd.businessService IN (:businessService)  ");
		 * preparedStatementValues.put("businessService",
		 * searchCriteria.getBusinessServices()); }
		 * 
		 ...
		 * preparedStatementValues.put("consumerCodes",
		 * searchCriteria.getConsumerCodes()); }
		 */
	 ...
	 */
	private void addPagination(StringBuilder query, Map<String, Object> preparedStatementValues,
