...
//						+ "  egpg.txn_amount,\n"
				...
				+ "  COUNT(DISTINCT CASE WHEN la.occupancy = 'Residential' AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				+ formattedDate + "', 'YYYY-MM-DD') THEN la.landinfoid END) AS Residential, \n"
				+ "  COUNT(DISTINCT CASE WHEN la.occupancy = 'Industrial' AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				+ formattedDate + "', 'YYYY-MM-DD') THEN la.landinfoid END) AS Industrial, \n"
				+ "  COUNT(DISTINCT CASE WHEN la.occupancy = 'Mercantile / Commercial' AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				+ formattedDate + "', 'YYYY-MM-DD') THEN la.landinfoid END) AS Commercial, \n"
				+ "  COUNT(DISTINCT CASE WHEN la.occupancy like ('%,%') AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				...
				+ formattedDate + "', 'YYYY-MM-DD') THEN bp.applicationno END) > 0\n"
				+ "  OR COUNT(DISTINCT CASE WHEN la.occupancy = 'Residential' AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				+ formattedDate + "', 'YYYY-MM-DD') THEN la.landinfoid END) > 0\n"
				+ "  OR COUNT(DISTINCT CASE WHEN la.occupancy = 'Industrial' AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				+ formattedDate + "', 'YYYY-MM-DD') THEN la.landinfoid END) > 0\n"
				+ "  OR COUNT(DISTINCT CASE WHEN la.occupancy = 'Mercantile / Commercial' AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
				+ formattedDate + "', 'YYYY-MM-DD') THEN la.landinfoid END) > 0\n"
				+ "  OR COUNT(DISTINCT CASE WHEN la.occupancy like('%,%') AND TO_TIMESTAMP(bp.approvaldate / 1000)::date = TO_DATE('"
...
//				+ "defunt,createdby,createddate) values (?,?,?,?,?,'" + date + "')";
		String insertQuery = "INSERT INTO eg_ndb_push_status(response_hash, no_of_reccords, error, environment, message_description, no_of_applications, approved_applications, data_pushed_date) values ('"
