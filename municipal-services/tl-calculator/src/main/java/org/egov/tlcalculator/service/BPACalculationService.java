...
//        demandService.generateDemand(calculationReq.getRequestInfo(), calculations, mdmsData, businessService_BPA);
	...
	private EstimatesAndSlabs getBaseTax(CalulationCriteria calulationCriteria, RequestInfo requestInfo,
		...
		String businessService = calulationCriteria.getTradelicense().getBusinessService();
		TaxHeadEstimate estimate = new TaxHeadEstimate();
		estimate.setEstimateAmount(billingSlab.getRate());
		estimate.setCategory(Category.FEE);
//		estimate.setTaxHeadCode(config.getBpabaseTaxHead());
		estimate.setTaxHeadCode(businessService.equals(businessService_BPA) ? config.getBpabaseTaxHead()
				: config.getBparenbaseTaxHead());
