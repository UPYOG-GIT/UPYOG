...
//		return null;
	...
	public Double getPenalty(String occupancyType, String tenantId, double area) {
		String sql = "select multiply_penalty from eg_rga_penalty where ulb_tenantid = '" + tenantId
				+ "'  and occupancy_type= '" + occupancyType + "' and from_val <= '" + area + "' and to_val >= '" + area + "'  ";
		log.info("RGARepository.getScategoryId Query : " + sql);
		return jdbcTemplate.queryForObject(sql, Double.class);
	}
	

	public List<Map<String, Object>> getDetailOfRgaSlabMaster(
			String tenantId, Double area, String occupancyType) {
		String sql = "select * from eg_rga_slab_master where ulb_tenantid = '" + tenantId + "'  and occupancy_type= '" + occupancyType + "' and from_val <= '" + area + "' and to_val >= '" + area + "' ";
		
		sql += " order by from_val";

		log.info("RGARepository.getDetailOfSlabMaster: " + sql);
...
//		return insertResult;
	...
	public void createRgaFeeDetail(PayTypeFeeDetailRequest payTypeFeeDetailRequest) {

		LocalDateTime date = LocalDateTime.now();

		String insertQuery = "insert into eg_rga_fee_details(feetype,srno,ulb_tenantid,"
				+ "application_no,charges_type_name,prop_plot_area,amount,rate,type,createddate)"
				+ "values (?,?,?,?,?,?,?,?,?,'" + date + "')";
		String insertBkQuery = "insert into eg_rga_fee_details_bkup(,feetype,srno,ulb_tenantid,"
				+ "application_no,charges_type_name,prop_plot_area,amount,rate,type,createddate)"
				+ "values (?,?,?,?,?,?,?,?,?,'" + date + "')";
		...
		List<Object[]> parameters = new ArrayList<Object[]>();
			...
			parameters.add(new Object[] {payTypeFeeDetailRequest.getFeeType(),
					count, payTypeFeeDetailRequest.getTenantId(), payTypeFeeDetailRequest.getApplicationNo(),
...
//			count += 1;
		...
		log.info("RGARepository.createFeeDetail: " + insertResult.length + " data inserted into eg_rga_fee_details table");
