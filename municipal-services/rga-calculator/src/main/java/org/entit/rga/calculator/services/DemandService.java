...
import org.entit.rga.calculator.repository.DemandRepository;
...
import org.entit.rga.calculator.web.models.demand.DemandDetail;
...
public class DemandService {
    ...
    @Autowired
    private FeeCalculationService mdmsService;
    ...
    public void generateDemand(RequestInfo requestInfo,List<Calculation> calculations,Object mdmsData){
        ...
        if(!CollectionUtils.isEmpty(calculations)){
            ...
            Set<String> applicationNos = calculations.stream().map(calculation -> calculation.getRga().getApplicationNo()).collect(Collectors.toSet());
//            Set<String> applicationNumbers = calculations.stream().map(calculation -> calculation.getRGA().getApplicationNo()).collect(Collectors.toSet());
            ...
            {      if(!applicationNumbersFromDemands.contains(calculation.getRga().getApplicationNo()))
    ...
    private List<Demand> updateDemand(RequestInfo requestInfo,List<Calculation> calculations){
        ...
        for(Calculation calculation : calculations) {
            ...
            List<Demand> searchResult = searchDemand(calculation.getTenantId(),Collections.singleton(calculation.getRga().getApplicationNo())
                    , requestInfo,calculation);

            if(CollectionUtils.isEmpty(searchResult))
                throw new CustomException(RGACalculatorConstants.INVALID_UPDATE,"No demand exists for applicationNumber: "+calculation.getRga().getApplicationNo());
    ...
    private List<Demand> searchDemand(String tenantId,Set<String> consumerCodes,RequestInfo requestInfo,Calculation calculation){
        ...
        uri = uri.replace("{2}",(utils.getBillingBusinessService(calculation.getRga().getBusinessService(), feeType)));
        ...
        try {
            ...
            throw new CustomException(RGACalculatorConstants.PARSING_ERROR,"Failed to parse response from Demand Search");
        }
    ...
    private List<Demand> createDemand(RequestInfo requestInfo,List<Calculation> calculations,Object mdmsData){
        ...
        for(Calculation calculation : calculations) {
            ...
            if(calculation.getRga()!=null) {
            	 	rga = calculation.getRga();
            } else if(calculation.getApplicationNumber()!=null) {
            		rga = rgaService.getBuildingPlan(requestInfo, calculation.getTenantId(), calculation.getApplicationNumber(), null);
            }
            
            if (rga == null)
                throw new CustomException(RGACalculatorConstants.INVALID_APPLICATION_NUMBER, "Demand cannot be generated for applicationNumber " +
                        calculation.getApplicationNumber() + "  Building plan application with this number does not exist ");

            String tenantId = calculation.getTenantId();
            String consumerCode = calculation.getRga().getApplicationNo();

            User owner = rga.getLandInfo().getOwners().get(0).toCommonUser();
...
//             addRoundOffTaxHead(calculation.getTenantId(),demandDetails);
            ...
            endCal.set(Calendar.DAY_OF_MONTH, 31);
             ...
             demands.add(Demand.builder()
                    ...
                    .consumerType("RGA")
                    .businessService(utils.getBillingBusinessService(rga.getBusinessService(),calculation.getFeeType()))
                    .build());
        }
    ...
    private void addRoundOffTaxHead(String tenantId,List<DemandDetail> demandDetails){
        ...
        * */
        for (DemandDetail demandDetail : demandDetails){
        	/*if(!demandDetail.getTaxHeadMasterCode().equalsIgnoreCase(RGACalculatorConstants.MDMS_ROUNDOFF_TAXHEAD))
     ...
     /*   if(prevRoundOffDemandDetail!=null){
        ...
        if(roundOff.compareTo(BigDecimal.ZERO)!=0){
                 DemandDetail roundOffDemandDetail = DemandDetail.builder()
                    .taxAmount(roundOff)
                    .taxHeadMasterCode(RGACalculatorConstants.MDMS_ROUNDOFF_TAXHEAD)
                    .tenantId(tenantId)
                    .collectionAmount(BigDecimal.ZERO)
                    .build();
