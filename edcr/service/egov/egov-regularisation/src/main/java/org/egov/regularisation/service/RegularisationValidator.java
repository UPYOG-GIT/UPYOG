...
import org.egov.regularisation.contract.RegComparisonRequest;
 ...
 */
@Service
public class RegularisationValidator {
    private static Logger LOG = LogManager.getLogger(RegularisationValidator.class);
    ...
    public ErrorDetail validate(final RegEdcrRequest edcr) {
        if (edcr != null) {
    ...
    public ErrorDetail validate(final RegComparisonRequest regComparisonRequest) {
        if (regComparisonRequest != null) {
            ErrorDetail error = new ErrorDetail();
            Field[] compaFields = regComparisonRequest.getClass().getDeclaredFields();
            ErrorDetail e1 = validateAttributes(regComparisonRequest, compaFields, error);
            if (e1 != null)
                return e1;
            if (regComparisonRequest.getRequestInfo() != null) {
                Field[] reqInfoFields = regComparisonRequest.getRequestInfo().getClass().getDeclaredFields();
                ErrorDetail e2 = validateAttributes(regComparisonRequest.getRequestInfo(), reqInfoFields, error);
                if (e2 != null)
                    return e2;
            }
            if (regComparisonRequest.getRequestInfo() != null && regComparisonRequest.getRequestInfo().getUserInfo() != null) {
                Field[] userInfoFields = regComparisonRequest.getRequestInfo().getUserInfo().getClass().getDeclaredFields();
                ErrorDetail e3 = validateAttributes(regComparisonRequest.getRequestInfo().getUserInfo(), userInfoFields, error);
                if (e3 != null)
                    return e3;
            }
        }
