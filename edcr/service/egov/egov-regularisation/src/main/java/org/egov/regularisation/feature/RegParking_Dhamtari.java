...
import static org.egov.regularisation.constants.DxfFileConstants.B;
import static org.egov.regularisation.constants.DxfFileConstants.C_MPBLC;
import static org.egov.regularisation.constants.DxfFileConstants.C_MPVT;
import static org.egov.regularisation.constants.DxfFileConstants.D_MHCT;
import static org.egov.regularisation.constants.DxfFileConstants.E;
import static org.egov.regularisation.constants.DxfFileConstants.F;
import static org.egov.regularisation.constants.DxfFileConstants.F_RT;
import static org.egov.regularisation.constants.DxfFileConstants.G;
import static org.egov.regularisation.constants.DxfFileConstants.H;
import static org.egov.regularisation.constants.DxfFileConstants.J;
import static org.egov.regularisation.constants.DxfFileConstants.PARKING_SLOT;
...
@Service
public class RegParking_Dhamtari extends FeatureProcess {

	private static final Logger LOGGER = LogManager.getLogger(RegParking_Dhamtari.class);
...
//	private static final String MAX_ALLOWED_MECH_PARK = "Maximum allowed mechanical parking";
	...
	private static final String RULE_7_C_19 = "Table 7-C-19";
...
//				)
				...
				setReportOutputDetails(pl, RULE_81, RULE_DESCRIPTION, requiredCarParkingArea + SQMTRS,
						totalProvidedCarParkingArea + SQMTRS, Result.Rejected.getResultVal());
			} else {
				setReportOutputDetails(pl, RULE_81, RULE_DESCRIPTION, requiredCarParkingArea + SQMTRS,
			...
			if (requiredVisitorParkArea > 0 && providedVisitorParkArea.compareTo(requiredVisitorParkingArea) < 0) {
				setReportOutputDetails(pl, RULE_81, RULE_DESCRIPTION, requiredVisitorParkingArea + SQMTRS,
						providedVisitorParkArea + SQMTRS, Result.Rejected.getResultVal());
	...
	private Double getRequiredCarParkArea(BigDecimal totalBuiltupArea, OccupancyTypeHelper mostRestrictiveOccupancy,
			...
			BigDecimal stiltParkingArea, BigDecimal lowerGroungFloorParkingArea, BigDecimal noOfBeds,
			Integer noOfSeats) {
...
//		BigDecimal totalProvidedCarParkArea = openParkingArea.add(coverParkingArea).add(basementParkingArea);
		...
		if (mostRestrictiveOccupancy != null && A.equals(mostRestrictiveOccupancy.getType().getCode())
			...
			// Academic
			if (totalBuiltupArea.compareTo(BigDecimal.valueOf(1000)) <= 0) {
			...
			requiredEcs = totalBuiltupArea.doubleValue() / 100;
		} else if (mostRestrictiveOccupancy != null && F_RT.equals(mostRestrictiveOccupancy.getSubtype().getCode())) {
			// Restaurants
			requiredEcs = totalBuiltupArea.doubleValue() / 100;
		} else if (mostRestrictiveOccupancy != null && C_MPVT.equals(mostRestrictiveOccupancy.getSubtype().getCode())) {
			// Medical Private
			requiredEcs = totalBuiltupArea.doubleValue() / 75;
			...
			requiredEcs = noOfSeats.doubleValue() / 150;
		}
