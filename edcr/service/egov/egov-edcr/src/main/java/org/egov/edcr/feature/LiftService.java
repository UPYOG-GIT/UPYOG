...
//                                                    .getCode()))) {
				 ...
				 * null && block.getBuilding().getBuildingHeight().intValue() > 16) { if
				 * (!block.getBuilding().getFloors().isEmpty()) { Integer floorUnits = 0; for
				 * (Floor floor : block.getBuilding().getFloors()) { floorUnits = floorUnits +
				 * floor.getUnits().size(); } if (floorUnits > 16) { boolean validOutside =
				 * false; Map<String, String> liftDimensions = new HashMap<>(); flr: for (Floor
				 * floor : block.getBuilding().getFloors()) { for (Lift lift : floor.getLifts())
				 * { if (lift.getLiftPolyLineClosed()) { for (Measurement measurement :
				 * lift.getLiftPolyLines()) { measurement.setWidth(BigDecimal.valueOf(
				 * Math.round(measurement.getWidth().doubleValue() * 100d) / 100d));
				 * measurement.setHeight(BigDecimal.valueOf(
				 * Math.round(measurement.getHeight().doubleValue() * 100d) / 100d)); if
				 * (measurement.getWidth().compareTo(BigDecimal.valueOf(1.1)) >= 0 &&
				 * measurement.getHeight().compareTo(BigDecimal.valueOf(2)) >= 0) { validOutside
				 * = true; liftDimensions.put("width", measurement.getWidth().toString());
				 * liftDimensions.put("length", measurement.getHeight().toString());
				 * liftDimensions.put("floor", floor.getNumber().toString());
				 * liftDimensions.put("lift", lift.getNumber().toString()); break flr; } } } } }
				 * if (validOutside) { setReportOutputDetails(plan, SUBRULE_118,
				 ...
				 * + " * " + liftDimensions.get("width"), Result.Accepted.getResultVal(),
				 * "Height of building is greater than 16 m", scrutinyDetail1); } else {
				 * setReportOutputDetails(plan, SUBRULE_118, SUBRULE_118_DESC, "2.0 m * 1.10 m",
				 * "None of the lift has minimum dimensions as provided",
