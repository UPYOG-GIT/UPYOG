...
@Service
public class GeneralStair_Demo extends GeneralStair {

	@Autowired
	EdcrRestService edcrRestService;

	private static final Logger LOG = LogManager.getLogger(GeneralStair_Demo.class);
	private static final String FLOOR = "Floor";
	private static final String RULE42_5_II = "42-5-ii";
//	private static final String RULE_77 = "77";
	private static final String RULE_77_1 = "77-1";
	private static final String RULE_77_2 = "77-2";
	private static final String RULE_77_3 = "77-3";
	private static final String RULE_77_4 = "77-4";
	private static final String RULE_77_5 = "77-5";
	...
	private static final String FLIGHT_POLYLINE_NOT_CLOSED_DESCRIPTION = "Flight polyline is not closed ";
...
//                                    }
	...
	private void validateLanding(Plan plan, Block block, ScrutinyDetail scrutinyDetailLanding,
		...
		ArrayList<Map<String, Object>> edcrRuleList = plan.getEdcrRuleList();
		for (StairLanding landing : landings) {
			...
			if (!widths.isEmpty()) {
				...
				if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
					...
					BigDecimal flightWidth = getRequiredWidth(block, mostRestrictiveOccupancyType, edcrRuleList);
					BigDecimal minimumWidth = flightWidth.multiply(BigDecimal.valueOf(2)).add(BigDecimal.valueOf(0.15));
					...
					if (valid) {
						setReportOutputDetailsFloorStairWise(plan, RULE_77_3, value,
				...
				if (flightPolyLines != null && flightPolyLines.size() > 0) {
					if (flightPolyLineClosed) {
						...
						errors.put("Flight PolyLine " + flightLayerName,
								FLIGHT_POLYLINE_NOT_CLOSED_DESCRIPTION + flightLayerName);
						plan.addErrors(errors);
					}
				} else {
					errors.put("Flight PolyLine " + flightLayerName,
	...
	private BigDecimal validateWidth(Plan plan, ScrutinyDetail scrutinyDetail2, Floor floor, Block block,
		...
		if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
			...
			if (valid) {
				setReportOutputDetailsFloorStairWise(plan, RULE_77_2, value,
...
//		BigDecimal buildingHeight = block.getBuilding().getHeight();
		...
		String featureName = "General Stair Width";
...
//			return BigDecimal.valueOf(1.5);
			...
			if (flight.getNoOfRises().compareTo(noOfFlights) > 0) {
				...
				if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
					...
					String value = typicalFloorValues.get("typicalFloors") != null
							...
							: " floor " + floor.getNumber();
					if (valid) {
						setReportOutputDetailsFloorStairWise(plan, RULE_77_4, value,
	...
	private BigDecimal getRequiredTread(OccupancyTypeHelper mostRestrictiveOccupancyType,
		...
		String occupancyName = "";
		String subOccupancyName = "";
		String featureName = "General Stair Tread Width";
		BigDecimal value = BigDecimal.ZERO;
...
//			String subOccupancyName = "";
			String featureName = "General Stair No of Risers";
...
//			valid = true;
			String value = typicalFloorValues.get("typicalFloors") != null
					...
					: " floor " + floor.getNumber();
			if (valid) {
//				setReportOutputDetailsFloorStairWise(plan, RULE_77_5, value,
