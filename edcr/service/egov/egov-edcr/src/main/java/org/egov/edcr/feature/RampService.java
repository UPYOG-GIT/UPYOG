...
//	private static final String SUBRULE_40_A3 = "40A(3)";
	/*
	 * private static final String SUBRULE_40_A1 = "40A(1)"; private static final
	 * String SUBRULE_40_A_7_DESC = "Minimum number of DA Rooms in block %s ";
	 * private static final String SUBRULE_40_A_3_WIDTH_DESC =
	 * "Minimum Width of Ramp %s for block %s "; private static final String
	 * SUBRULE_40_DESC = "Maximum slope of ramp %s for block %s ";
	 */
	private static final String SUBRULE_50_C_4_B_DESCRIPTION = "Maximum slope of ramp %s";

	/*
	 * private static final String SUBRULE_40_A_1_DESC = "DA Ramp"; private static
	 * final String SUBRULE_40_A_3_SLOPE_DESC =
...
//	 private static final String SUBRULE_40_A_3_WIDTH_DESCRIPTION = "Minimum Width of Ramp %s";
	...
	public Plan validate(Plan pl) {
						...
						for (Ramp ramp : ramps) {
							...
							if (rampPolyLines != null && !rampPolyLines.isEmpty()) {
								validateDimensions(pl, block.getNumber(), floor.getNumber(),
										ramp.getNumber().toString(), rampPolyLines);
							}
						}
					}
				}
		...
		if (pl != null && !pl.getBlocks().isEmpty()) {
			blk: for (Block block : pl.getBlocks()) {
				 ...
				 * !block.getBuilding().getOccupancies().isEmpty()) { if
				 * (Util.checkExemptionConditionForBuildingParts(block)) { continue blk; }
				 * List<OccupancyType> occupancyTypeList =
				 * block.getBuilding().getOccupancies().stream() .map(occupancy ->
				 * occupancy.getType()).collect(Collectors.toList()); for (OccupancyType
				 * occupancyType : occupancyTypeList) { if
				 ...
				 * String[]{String.format(DcrConstants.RAMP, block.getNumber())},
				 * LocaleContextHolder.getLocale())); pl.addErrors(errors); break; } } } }
				 */
						...
						if (!isSlopeDefined) {
							errors.put(String.format(DcrConstants.RAMP_SLOPE, "", block.getNumber()),
									edcrMessageSource.getMessage(DcrConstants.OBJECTNOTDEFINED,
											new String[] {
													String.format(DcrConstants.RAMP_SLOPE, "", block.getNumber()) },
	...
	private boolean getOccupanciesForRamp(OccupancyType occupancyType) {
		return occupancyType.equals(OccupancyType.OCCUPANCY_A2) || occupancyType.equals(OccupancyType.OCCUPANCY_A3)
				|| occupancyType.equals(OccupancyType.OCCUPANCY_A4) || occupancyType.equals(OccupancyType.OCCUPANCY_B1)
...
//		validate(pl);
		...
		if (pl != null && !pl.getBlocks().isEmpty()) {
			blk: for (Block block : pl.getBlocks()) {
				...
				OccupancyTypeHelper mostRestrictiveOccupancyType = pl.getVirtualBuilding()
				...
				if (block.getBuilding() != null && !block.getBuilding().getOccupancies().isEmpty()) {
					 ...
					 */
					/*
					 * for (OccupancyType occupancyType : occupancyTypeList) { if
					 * (getOccupanciesForRamp(occupancyType)) { if (!block.getDARamps().isEmpty()) {
					 * setReportOutputDetails(pl, SUBRULE_40_A1, SUBRULE_40_A_1_DESC, "",
					 * DcrConstants.OBJECTDEFINED_DESC, Result.Accepted.getResultVal(),
					 * scrutinyDetail); break; } else { setReportOutputDetails(pl, SUBRULE_40_A1,
					 * SUBRULE_40_A_1_DESC, "", DcrConstants.OBJECTNOTDEFINED_DESC,
					 ...
					 * !A_R.equalsIgnoreCase(mostRestrictiveOccupancyType.getSubtype().getCode())) {
					 * if (!block.getDARamps().isEmpty()) { boolean isSlopeDefined = false; for
					 * (DARamp daRamp : block.getDARamps()) { if (daRamp != null &&
					 * daRamp.getSlope() != null &&
					 * daRamp.getSlope().compareTo(BigDecimal.valueOf(0)) > 0) { isSlopeDefined =
					 * true; } } if (isSlopeDefined) { setReportOutputDetails(pl, SUBRULE_50_C_4_B,
					 * SUBRULE_50_C_4_B_SLOPE_MAN_DESC, "", DcrConstants.OBJECTDEFINED_DESC,
					 * Result.Accepted.getResultVal(), scrutinyDetail1); } else {
					 ...
					 * scrutinyDetail1); } valid = false; if (isSlopeDefined) { Map<String, String>
					 * mapOfRampNumberAndSlopeValues = new HashMap<>(); BigDecimal expectedSlope =
					 * BigDecimal.valueOf(1).divide(BigDecimal.valueOf(12), 2,
					 * RoundingMode.HALF_UP); for (DARamp daRamp : block.getDARamps()) { BigDecimal
					 * slope = daRamp.getSlope(); if (slope != null &&
					 * slope.compareTo(BigDecimal.valueOf(0)) > 0 && expectedSlope != null) { if
					 * (slope.compareTo(expectedSlope) <= 0) { valid = true;
					 * mapOfRampNumberAndSlopeValues.put("daRampNumber",
					 * daRamp.getNumber().toString()); mapOfRampNumberAndSlopeValues.put("slope",
					 * slope.toString()); break; } } } if (valid) { setReportOutputDetails(pl,
					 * SUBRULE_50_C_4_B, String.format(SUBRULE_50_C_4_B_SLOPE_DESCRIPTION,
					 ...
					 * scrutinyDetail2); } else { setReportOutputDetails(pl, SUBRULE_50_C_4_B,
					...
					if (block.getBuilding().getBuildingHeight().compareTo(new BigDecimal(15)) > 0) {
						...
						if (block.getBuilding() != null && !block.getBuilding().getFloors().isEmpty()) {
							for (Floor floor : block.getBuilding().getFloors()) {
								 ...
								 * floor.getConvertedOccupancies()) { if
								 * (occupancy.getType().equals(OccupancyType.OCCUPANCY_A2) &&
								 * !floor.getDaRooms().isEmpty() && !floor.getUnits().isEmpty()) { valid =
								 * false; int noOfDaRooms = floor.getDaRooms().size(); int
								 * noOfFloorUnitsInColorCode23And3 = 0; if (!floor.getUnits().isEmpty()) { for
								 * (FloorUnit floorUnit : floor.getUnits()) { if (floorUnit != null &&
								 * (floorUnit.getColorCode() == 23 || floorUnit.getColorCode() == 3)) {
								 * noOfFloorUnitsInColorCode23And3++; } } } if (noOfFloorUnitsInColorCode23And3
								 * != 0) { BigDecimal expectedNoOfDARooms = BigDecimal
								 * .valueOf(Double.valueOf(String.valueOf(noOfFloorUnitsInColorCode23And3)))
								 * .divide(BigDecimal.valueOf(25), 2, RoundingMode.HALF_UP); boolean
								 * isTypicalRepititiveFloor = false; Map<String, Object> typicalFloorValues =
								 * Util.getTypicalFloorValues(block, floor, isTypicalRepititiveFloor); if
								 * (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) { if
								 * (BigDecimal.valueOf(Double.valueOf(String.valueOf(noOfDaRooms)))
								 ...
								 * (valid) { setReportOutputDetailsFloorWise(pl, SUBRULE_40_A7, value,
								 * expectedNoOfDARooms.toString(), String.valueOf(noOfDaRooms),
								 * Result.Accepted.getResultVal(), scrutinyDetail3); } else {
								 * setReportOutputDetailsFloorWise(pl, SUBRULE_40_A7, value,
								 * expectedNoOfDARooms.toString(), String.valueOf(noOfDaRooms),
									...
									if (ramp.getRampClosed()) {
										...
										if (!rampWidths.isEmpty()) {
											...
											if (!(Boolean) typicalFloorValues.get("isTypicalRepititiveFloor")) {
												...
												if (valid) {
													setReportOutputDetailsFloorWiseWithDescription(pl, SUBRULE_50_C_4_B,
										...
										/*
										 * BigDecimal rampTotalLength = BigDecimal.ZERO; for (BigDecimal length :
										 * rampLengths) { rampTotalLength = rampTotalLength.add(length); } if
										 * (rampTotalLength.compareTo(BigDecimal.valueOf(0)) > 0 &&
										 * ramp.getFloorHeight() != null) { boolean isTypicalRepititiveFloor = false;
										 * BigDecimal rampSlope = ramp.getFloorHeight().divide(rampTotalLength, 2,
										 * RoundingMode.HALF_UP); ramp.setSlope(rampSlope); BigDecimal expectedSlope =
										 * BigDecimal.ZERO; if (mostRestrictiveFarHelper != null &&
										 * ((mostRestrictiveFarHelper.getType() != null &&
										 ...
										 * mostRestrictiveFarHelper.getSubtype().getCode()
										 * .equalsIgnoreCase(DxfFileConstants.C_MOP))))) { expectedSlope =
										 * BigDecimal.valueOf(0.05); } else { expectedSlope = ramp.getFloorHeight()
										 * .compareTo(BigDecimal.valueOf(2.4)) > 0 ? BigDecimal.valueOf(0.05) :
										 * BigDecimal.valueOf(0.08); } valid = false; Map<String, Object>
										 * typicalFloorValues = Util.getTypicalFloorValues(block, floor,
										 * isTypicalRepititiveFloor); if (!(Boolean)
										 * typicalFloorValues.get("isTypicalRepititiveFloor")) { if
										 * (rampSlope.compareTo(expectedSlope) <= 0) { valid = true; } String value =
										 * typicalFloorValues.get("typicalFloors") != null ? (String)
										 * typicalFloorValues.get("typicalFloors") : " floor " + floor.getNumber(); if
										 * (valid) { setReportOutputDetailsFloorWiseWithDescription(pl, SUBRULE_40,
										 * String.format(SUBRULE_50_C_4_B_DESCRIPTION, ramp.getNumber()), value,
										 * expectedSlope.toString(), rampSlope.toString(),
										 * Result.Accepted.getResultVal(), scrutinyDetail5); } else {
