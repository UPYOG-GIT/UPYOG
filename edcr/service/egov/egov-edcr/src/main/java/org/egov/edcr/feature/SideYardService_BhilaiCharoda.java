...
@Service
public class SideYardService_BhilaiCharoda extends SideYardService {

	private static final Logger LOG = LogManager.getLogger(SideYardService_BhilaiCharoda.class);
	...
	public void processSideYard(final Plan pl) {
		...
		LOG.info("inside SideYardYardService_BhilaiCharoda process");
...
//				LOG.info(pl.getPlanInformation().getRoadWidth());
...
//					scrutinyDetail.setKey("Block_" + block.getName() + "_" + "Side Setback");
					if (sideYard1 != null || sideYard2 != null) {
						...
						if (side1RoadReservePresent) {
							side1RoadReserve = roadReserve.get(side1RoadReserveIndex).getShortestDistanceToRoad().get(0)
									.doubleValue();
						}
						if (side2RoadReservePresent) {
							side2RoadReserve = roadReserve.get(side2RoadReserveIndex).getShortestDistanceToRoad().get(0)
									.doubleValue();
						}
...
//								&& pl.getPlanInformation().getWidthOfPlot().compareTo(BigDecimal.valueOf(13)) <= 0) {
...
//						}
						...
						if (pl.getPlanInformation() != null && occupancy.getTypeHelper().getType() != null) {
							if (A.equalsIgnoreCase(occupancy.getTypeHelper().getType().getCode())) {
								if (pl.getPlanInformation().getWidthOfPlot().compareTo(BigDecimal.valueOf(12)) <= 0) {
									exemptSideYard1ForAAndF(pl, block, sideYard1Result);
								}
								if (pl.getPlanInformation().getWidthOfPlot().compareTo(BigDecimal.valueOf(7.5)) <= 0) {
									exemptSideYard2ForAAndF(pl, block, sideYard2Result);
								}
							} else if (F.equalsIgnoreCase(occupancy.getTypeHelper().getType().getCode()) && pl
									.getPlanInformation().getWidthOfPlot().compareTo(BigDecimal.valueOf(12)) <= 0) {

								exemptSideYard1ForAAndF(pl, block, sideYard1Result);
								exemptSideYard2ForAAndF(pl, block, sideYard2Result);

							} else if (J.equalsIgnoreCase(occupancy.getTypeHelper().getType().getCode()) && pl
									.getPlanInformation().getWidthOfPlot().compareTo(BigDecimal.valueOf(7.5)) <= 0) {
								exemptSideYard1ForAAndF(pl, block, sideYard1Result);
							}
						}
						addSideYardResult(pl, errors, sideYard1Result, sideYard2Result);
...
//					&& pl.getPlanInformation().getRoadWidth().compareTo(ROAD_WIDTH_TWELVE_POINTTWO) < 0
			...
			checkCommercial(pl, blockName, level, min, max, minMeanlength, maxMeanLength, mostRestrictiveOccupancy,
					...
					buildingHeight, roadWidth, block);
		}
	}
	...
	private void checkResidential(Plan pl, String blockName, Integer level, final double min, final double max,
			...
			BigDecimal roadWidth, Block block) {
		...
		if (buildingHeight.compareTo(BigDecimal.valueOf(9.5)) <= 0
			...
			} else if (widthOfPlot.compareTo(BigDecimal.valueOf(7.50)) > 0
					&& widthOfPlot.compareTo(BigDecimal.valueOf(9.0)) <= 0) {
				side2val = BigDecimal.ZERO;
				side1val = SIDEVALUE_TWO;
			} else if (widthOfPlot.compareTo(BigDecimal.valueOf(9.0)) > 0
					&& widthOfPlot.compareTo(BigDecimal.valueOf(12.0)) <= 0) {
				side2val = BigDecimal.ZERO;
				side1val = SIDEVALUE_THREE;
			} else if (widthOfPlot.compareTo(BigDecimal.valueOf(12.0)) > 0
					&& widthOfPlot.compareTo(BigDecimal.valueOf(18.0)) <= 0) {
				side2val = SIDEVALUE_ONEPOINTFIVE;
				side1val = SIDEVALUE_THREE;
			} else if (widthOfPlot.compareTo(BigDecimal.valueOf(18.0)) > 0
					&& widthOfPlot.compareTo(BigDecimal.valueOf(24.0)) <= 0) {
				side2val = SIDEVALUE_TWOPOINTTWOFIVE;
				side1val = SIDEVALUE_THREEPOINTFIVE;
			} else if (widthOfPlot.compareTo(BigDecimal.valueOf(24.0)) > 0) {
