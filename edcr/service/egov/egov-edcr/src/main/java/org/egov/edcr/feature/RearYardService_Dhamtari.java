...
@Service
public class RearYardService_Dhamtari extends RearYardService {

	private static final Logger LOG = LogManager.getLogger(RearYardService_Dhamtari.class);
...
//							scrutinyDetail.setKey("Block_" + block.getName() + "_" + "Rear Setback");
							...
							if (occupancy.getTypeHelper().getType() != null
								...
								checkRearYard(pl, block.getBuilding(), block, setback.getLevel(), plot, REAR_YARD_DESC,
										min, mean, occupancy.getTypeHelper(), rearYardResult, buildingHeight);
...
//								}
								...
								checkRearYardForIndustrial(setback, block.getBuilding(), pl, block, setback.getLevel(),
										plot, REAR_YARD_DESC, min, mean, occupancy.getTypeHelper(), rearYardResult);
							}
...
//							} // for end
						...
						if (pl.getPlanInformation() != null && occupancy.getTypeHelper().getType() != null) {
							if (A.equalsIgnoreCase(occupancy.getTypeHelper().getType().getCode())
									&& pl.getPlanInformation().getDepthOfPlot().compareTo(BigDecimal.valueOf(8)) <= 0) {
...
//					&& pl.getPlanInformation().getRoadWidth().compareTo(ROAD_WIDTH_TWELVE_POINTTWO) < 0
	...
	private Boolean residential(Block block, Integer level, final BigDecimal min, final BigDecimal mean,
		 ...
		 * widthOfPlot.compareTo(BigDecimal.valueOf(15)) <= 0) { minVal =
		 ...
		 * widthOfPlot.compareTo(BigDecimal.valueOf(30)) <= 0) { minVal =
		 * REARYARDMINIMUM_DISTANCE_4_5; } else { minVal = REARYARDMINIMUM_DISTANCE_4_5;
		 * }
		 */
