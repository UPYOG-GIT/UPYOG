...
public class RecreationalSpace extends FeatureProcess {
	...
	public Plan validate(Plan pl) {/*
									 * HashMap<String, String> errors = new HashMap<>(); for (Block block :
									 * pl.getBlocks()) { BigDecimal totalRecreationArea = BigDecimal.ZERO; int
									 * numberOfUnitsInBlock = 0; if (block.getBuilding() != null) { if
									 ...
									 * checkOccupancyPresent(block.getBuilding().getOccupancies(),
									 * OccupancyType.OCCUPANCY_A4)) { for (Floor floor :
									 * block.getBuilding().getFloors()) { for (FloorUnit unit : floor.getUnits()) {
									 ...
									 * totalRecreationArea.add(measure.getArea()); } } } } } if
									 * (numberOfUnitsInBlock > TOTALNUMBEROFUNITS &&
									 * totalRecreationArea.compareTo(BigDecimal.ZERO) <= 0) {
									 * errors.put(RECREATIONSPACE_DESC, getLocaleMessage(OBJECTNOTDEFINED,
									 * RECREATIONSPACE_DESC) + " for block " + block.getNumber());
									 * pl.addErrors(errors); } } } }
									 */
...
//			if (occ.getType().equals(occupancyType))
			if (occ.getTypeHelper().getSubtype()!=null && occ.getTypeHelper().getSubtype().equals(occupancyType))
				return true;
		}
		return false;
	}
	...
	public Plan process(Plan pl) {
		...
		for (Block block : pl.getBlocks()) {
			...
			if(!isReport) {
				...
				scrutinyDetail1.setKey("Block_" + block.getName() + "_" + "Recreational Space");
				Map<String, String> details = new HashMap<>();
				details.put(DESCRIPTION, "Recreational Space");
				details.put(PROVIDED, "Recreational Space Not Provided");
				details.put(STATUS, "");
